    ...
    public function render_settings_page() {
        ...
        <div class="wrap intersoccer-admin">
            ...
            jQuery(document).ready(function($) {
                // Credit value (1 point = X CHF at checkout)
                const creditValue = <?php echo esc_js($credit_value); ?>;

                // Update preview calculations and percentage when rate inputs change
                function updatePreview($input) {
                    const rate = parseInt($input.val()) || 10;
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $message.html('<div class="notice notice-error"><p>' + (response.data?.message || 'Error saving rates') + '</p></div>').show();
                            }
                        },
                        error: function() {
                        ...
                        complete: function() {
                            $button.prop('disabled', false).html('<span class="dashicons dashicons-saved"></span> <?php esc_html_e('Save Points Configuration & Rates', 'intersoccer-referral'); ?>');
                        }
                    });
                });
            ...
            jQuery(document).ready(function($) {
                ...
                // Initialize tier indexes for each role
                roles.forEach(function(role) {
                    tierIndexes[role] = $('.commission-tier-row[data-role="' + role + '"]').length;
                });
                
                // Update tier example when rate changes
                function updateTierExample($input) {
                    ...
                    if ($example.length === 0) {
                        // Create example span if it doesn't exist
                ...
                // Update examples on rate input change
                $(document).on('input', '.tier-rate', function() {
                    updateTierExample($(this));
                });
                
                // Add new tier for specific role
                $(document).on('click', '.add-commission-tier', function() {
                    ...
                    const tierRow = $('<div class="commission-tier-row" data-role="' + role + '" data-tier-index="' + tierIndex + '" style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; padding: 10px; background: #f9f9f9; border-radius: 4px;">' +
                ...
                $(document).on('click', '.delete-tier', function() {
                    ...
                    if (container.find('.commission-tier-row').length <= 1) {
                        alert('<?php esc_html_e('You must have at least one tier.', 'intersoccer-referral'); ?>');
                        return;
                    }
                    $row.remove();
                });
                ...
                $('#save-all-commission-tiers').on('click', function() {
                    ...
                    roles.forEach(function(role) {
                        ...
                        $('.commission-tier-row[data-role="' + role + '"]').each(function() {
                            const min = parseInt($(this).find('.tier-min-customers').val()) || 1;
                            const maxInput = $(this).find('.tier-max-customers').val();
                            const max = maxInput === '' ? 999999 : parseInt(maxInput);
                            const rate = parseFloat($(this).find('.tier-rate').val()) || 0;
                            
                            if (min < 1 || rate < 0 || rate > 100) {
                                alert('<?php esc_html_e('Please enter valid values: Min >= 1, Rate between 0-100.', 'intersoccer-referral'); ?>');
                                return false;
                            }
                        ...
                        if (tiers.length === 0) {
                            alert('<?php esc_html_e('Please add at least one tier for each role.', 'intersoccer-referral'); ?>');
                            return false;
                        }
                    ...
                    $button.prop('disabled', true).text('<?php esc_html_e('Saving...', 'intersoccer-referral'); ?>');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                // Show success message for each role
                                roles.forEach(function(role) {
                                ...
                                roles.forEach(function(role) {
                                    ...
                                    $message.removeClass('notice-success').addClass('notice notice-error')
                                        .html('<p>' + (response.data.message || '<?php esc_html_e('Error saving tiers.', 'intersoccer-referral'); ?>') + '</p>').show();
                                });
                            }
                        },
                        error: function() {
                            roles.forEach(function(role) {
                                const $message = $('#' + role + '-commission-tiers-message');
                                $message.removeClass('notice-success').addClass('notice notice-error')
                                    .html('<p><?php esc_html_e('Error saving tiers.', 'intersoccer-referral'); ?></p>').show();
                            });
                        },
                        complete: function() {
                            $button.prop('disabled', false).text('<?php esc_html_e('Save All Commission Tiers', 'intersoccer-referral'); ?>');
                        }
                    });
                });
            });
    ...
    public function render_tools_page() {
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '#simulator-load-recommendations', function(e) {
                ...
                if (!dateFrom || !dateTo) {
                    alert('<?php echo esc_js(__('Please select both start and end dates first', 'intersoccer-referral')); ?>');
                    return;
                }
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        nonce: '<?php echo wp_create_nonce('intersoccer_simulator_nonce'); ?>',
                        date_from: dateFrom,
                        date_to: dateTo
                    },
                    dataType: 'json',
                    success: function(response) {
                        ...
                        if (response.success) {
                            let html = '<h4><?php echo esc_js(__('Recommendations Based on Historical Data', 'intersoccer-referral')); ?></h4>';
                            ...
                            if (response.data.recommendations && response.data.recommendations.length > 0) {
                                response.data.recommendations.forEach(function(rec) {
                                    ...
                                    html += '<span style="color: #666;"><?php echo esc_js(__('Current:', 'intersoccer-referral')); ?> ' + rec.current + ' | <?php echo esc_js(__('Recommended:', 'intersoccer-referral')); ?> ' + rec.recommended + '</span><br>';
                                    html += '<small style="color: #888;">' + rec.reason + '</small>';
                                    html += '</div>';
                                });
                            } else {
                                html += '<p><?php echo esc_js(__('No specific recommendations. Your current settings appear to be well-optimized.', 'intersoccer-referral')); ?></p>';
                            }
                            
                            html += '<hr style="margin: 15px 0;">';
                            html += '<h5><?php echo esc_js(__('Historical Data Summary', 'intersoccer-referral')); ?></h5>';
                            html += '<table class="widefat" style="margin-top: 10px;">';
                            html += '<tr><th style="width: 200px;"><?php echo esc_js(__('Total Orders', 'intersoccer-referral')); ?></th><td>' + response.data.historical.total_orders + '</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Total Referrals', 'intersoccer-referral')); ?></th><td>' + response.data.historical.total_referrals + '</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Actual Referral Rate', 'intersoccer-referral')); ?></th><td>' + response.data.historical.actual_referral_rate.toFixed(1) + '%</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Coach Referral Rate', 'intersoccer-referral')); ?></th><td>' + response.data.historical.coach_referral_rate.toFixed(1) + '%</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Customer Referral Rate', 'intersoccer-referral')); ?></th><td>' + response.data.historical.customer_referral_rate.toFixed(1) + '%</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Average Commission', 'intersoccer-referral')); ?></th><td>CHF ' + response.data.historical.avg_commission.toFixed(2) + '</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Average Order Value', 'intersoccer-referral')); ?></th><td>CHF ' + response.data.historical.avg_order_value.toFixed(2) + '</td></tr>';
                            html += '<tr><th><?php echo esc_js(__('Active Coaches', 'intersoccer-referral')); ?></th><td>' + response.data.historical.coach_count + '</td></tr>';
                            html += '</table>';
                            ...
                            alert(response.data.message || '<?php echo esc_js(__('Error loading recommendations', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        ...
                        alert('<?php echo esc_js(__('Error loading recommendations', 'intersoccer-referral')); ?>');
                    }
                });
            });
            ...
            $(document).on('click', '.simulator-add-tier', function() {
                ...
                const html = '<div class="simulator-tier-row" style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 8px; background: #fff; border-radius: 3px;">' +
                    ...
                    '<span><?php echo esc_js(__('to', 'intersoccer-referral')); ?></span>' +
                    '<input type="number" class="simulator-tier-max" value="" min="1" style="width: 70px;" placeholder="Max">' +
                    '<span><?php echo esc_js(__('customers:', 'intersoccer-referral')); ?></span>' +
                    '<input type="number" class="simulator-tier-rate" value="' + lastRate + '" min="0" max="100" step="0.1" style="width: 70px;" placeholder="%">' +
                    '<span>%</span>' +
                    '<button type="button" class="button button-small simulator-remove-tier" style="margin-left: auto;"><?php echo esc_js(__('Remove', 'intersoccer-referral')); ?></button>' +
                    '</div>';
            ...
            $(document).on('click', '.simulator-remove-tier', function() {
                if ($(this).closest('.simulator-commission-tiers').find('.simulator-tier-row').length > 1) {
                    ...
                    alert('<?php echo esc_js(__('At least one tier is required', 'intersoccer-referral')); ?>');
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            alert(response.data.message || '<?php echo esc_js(__('Error loading settings', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Load settings error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error loading settings', 'intersoccer-referral')); ?>');
                    }
                });
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            alert(response.data.message || '<?php echo esc_js(__('Error loading template', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Template error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error loading template', 'intersoccer-referral')); ?>');
                    }
                });
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            alert(response.data.message || '<?php echo esc_js(__('Error loading scenario', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Load scenario error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error loading scenario', 'intersoccer-referral')); ?>');
                    }
                });
            });
            ...
            $(document).on('click', '#simulator-save-scenario', function(e) {
                ...
                if (!scenarioName) {
                    alert('<?php echo esc_js(__('Please enter a scenario name', 'intersoccer-referral')); ?>');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            alert('<?php echo esc_js(__('Scenario saved successfully', 'intersoccer-referral')); ?>');
                            loadScenarios();
                        } else {
                            alert(response.data.message || '<?php echo esc_js(__('Error saving scenario', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Save scenario error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error saving scenario', 'intersoccer-referral')); ?>: ' + (xhr.responseJSON?.data?.message || error));
                    }
                });
            });
            ...
            $(document).on('click', '#simulator-delete-scenario', function(e) {
                ...
                if (!scenarioId || !confirm('<?php echo esc_js(__('Are you sure you want to delete this scenario?', 'intersoccer-referral')); ?>')) {
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            alert('<?php echo esc_js(__('Scenario deleted successfully', 'intersoccer-referral')); ?>');
                            $('#simulator-load-scenario').val('');
                            $('#simulator-delete-scenario').hide();
                            loadScenarios();
                        } else {
                            alert(response.data.message || '<?php echo esc_js(__('Error deleting scenario', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Delete scenario error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error deleting scenario', 'intersoccer-referral')); ?>');
                    }
                });
            });
            ...
            $(document).on('click', '#simulator-run', function(e) {
                ...
                const dateTo = $('#simulator-date-to').val();
                if (!dateFrom || !dateTo) {
                    alert('<?php echo esc_js(__('Please select both start and end dates', 'intersoccer-referral')); ?>');
                    return;
                }
                if (dateFrom > dateTo) {
                    alert('<?php echo esc_js(__('Start date must be before end date', 'intersoccer-referral')); ?>');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            alert(response.data.message || '<?php echo esc_js(__('Error running simulation', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        $spinner.removeClass('is-active');
                        console.error('Simulation error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error running simulation', 'intersoccer-referral')); ?>: ' + (xhr.responseJSON?.data?.message || error));
                    }
                });
            });
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            if (response.data.sensitivity_data && typeof Chart !== 'undefined') {
                                ...
                                alert('<?php echo esc_js(__('No sensitivity data returned', 'intersoccer-referral')); ?>');
                            }
                        } else {
                            alert(response.data.message || '<?php echo esc_js(__('Error running sensitivity analysis', 'intersoccer-referral')); ?>');
                        }
                    },
                    error: function(xhr, status, error) {
                        $spinner.removeClass('is-active');
                        console.error('Sensitivity analysis error:', status, error, xhr.responseText);
                        alert('<?php echo esc_js(__('Error running sensitivity analysis', 'intersoccer-referral')); ?>: ' + (xhr.responseJSON?.data?.message || error));
                    }
                });
            });
                ...
                if (referralsData.labels && referralsData.customer && referralsData.coach) {
                    ...
                    simulatorReferralsChart = new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: '<?php echo esc_js(__('Customer Referrals', 'intersoccer-referral')); ?>',
                                ...
                                label: '<?php echo esc_js(__('Coach Referrals', 'intersoccer-referral')); ?>',
                        ...
                        options: {
                            ...
                            scales: {
                                y: {
                                    ...
                                    title: {
                                        display: true,
                                        text: '<?php echo esc_js(__('Number of Referrals', 'intersoccer-referral')); ?>'
                                    }
                                },
                                x: {
                                    title: {
                                        display: true,
                                        text: '<?php echo esc_js(__('Month', 'intersoccer-referral')); ?>'
                                    }
                                }
                            }
                        }
                    ...
                    simulatorReferralsChart = new Chart(ctx, {
                        ...
                        data: {
                            labels: [
                                '<?php echo esc_js(__('Customer Referrals', 'intersoccer-referral')); ?>',
                                '<?php echo esc_js(__('Coach Referrals', 'intersoccer-referral')); ?>'
                            ],
                            datasets: [{
            ...
            function initializeCommissionsChart(commissionsData) {
                ...
                if (commissionsData.labels && commissionsData.coach && commissionsData.partner) {
                    ...
                    if (commissionsData.coach && commissionsData.coach.length > 0) {
                        datasets.push({
                            label: '<?php echo esc_js(__('Coach', 'intersoccer-referral')); ?>',
                    ...
                    if (commissionsData.partner && commissionsData.partner.length > 0) {
                        datasets.push({
                            label: '<?php echo esc_js(__('Partner', 'intersoccer-referral')); ?>',
                    ...
                    if (commissionsData.social_influencer && commissionsData.social_influencer.length > 0) {
                        datasets.push({
                            label: '<?php echo esc_js(__('Social Influencer', 'intersoccer-referral')); ?>',
                        ...
                        options: {
                            ...
                            scales: {
                                y: {
                                    ...
                                    title: {
                                        display: true,
                                        text: '<?php echo esc_js(__('Commissions (CHF)', 'intersoccer-referral')); ?>'
                                    },
                                    ticks: {
                                        callback: function(value) {
                                            return 'CHF ' + value.toLocaleString('en-US', {
                    ...
                    if ((commissionsData.total_social_influencer || commissionsData.social_influencer) > 0) {
                        labels.push('<?php echo esc_js(__('Social Influencer', 'intersoccer-referral')); ?>');
                    ...
                    simulatorCommissionsChart = new Chart(ctx, {
                        ...
                        data: {
                            labels: labels,
                            datasets: [{
                                label: '<?php echo esc_js(__('Commissions (CHF)', 'intersoccer-referral')); ?>',
                                data: data,
            ...
            function initializeSensitivityChart(sensitivityData) {
                ...
                simulatorSensitivityChart = new Chart(ctx, {
                    ...
                    data: {
                        labels: labels,
                        datasets: [{
                            label: '<?php echo esc_js(__('Impact on Net Profit (%)', 'intersoccer-referral')); ?>',
                            data: impacts,
                    ...
                    options: {
                        ...
                        scales: {
                            x: {
                                ...
                                title: {
                                    display: true,
                                    text: '<?php echo esc_js(__('Impact on Net Profit (%)', 'intersoccer-referral')); ?>'
                                },
                                ticks: {
                                    callback: function(value) {
                                        return value + '%';
                            ...
                            y: {
                                title: {
                                    display: true,
                                    text: '<?php echo esc_js(__('Variable', 'intersoccer-referral')); ?>'
                                }
                            }
                        }
                    }
            ...
            $(document).on('click', '#simulator-export-excel', function(e) {
                ...
                const settings = collectSimulatorSettings();
                window.location.href = ajaxurl + '?action=intersoccer_export_simulator_excel&nonce=<?php echo wp_create_nonce('intersoccer_simulator_nonce'); ?>&settings=' + encodeURIComponent(JSON.stringify(settings));
            });
            ...
            $(document).on('click', '#simulator-export-pdf', function(e) {
                ...
                const settings = collectSimulatorSettings();
                window.location.href = ajaxurl + '?action=intersoccer_export_simulator_pdf&nonce=<?php echo wp_create_nonce('intersoccer_simulator_nonce'); ?>&settings=' + encodeURIComponent(JSON.stringify(settings));
            });
        });
        </script>
        <?php
